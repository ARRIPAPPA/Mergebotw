...
async def MergeSub(filePath: str, subPath: str, user_id):
    ...
    subTrack += 1
    subTitle = f"Track {subTrack} - Join Our TG Channel @Blaster_Originals"
...
def MergeSubNew(filePath: str, subPath: str, user_id, file_list):
    ...
    for j in range(1, (len(file_list))):
        ...
        muxcmd.append(f"{j}:s")
        muxcmd.append(f"-metadata:s:s:{subTrack}")
        muxcmd.append(f"title=Track {subTrack+1} - Join Our TG Channel @Blaster_Originals")
        subTrack += 1
    muxcmd.append("-c:v")
    muxcmd.append("copy")
    muxcmd.append("-c:a")
...
def MergeAudio(videoPath: str, files_list: list, user_id):
    ...
    LOGGER.info(process)
    return f"downloads/{str(user_id)}/(@ABlaster_Originals)_export.mkv"
